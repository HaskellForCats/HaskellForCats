...
Ryan Trinkle former C.T.O. @ SkedgeMe
  Author of the https://github.com/ryantrinkle/try-reflex
  a Haskell to JS library to Deliver FRP to Front End Web Development.    

API into Sephora Website 
Initial JAVA Code Base = 128 cores on AWS.
2 machines, 32-cores each (64-threads each with hyperthreading)
 
------- 41-K.L.O.C. -------- No sure what this metric means 
41,000 lines of source code 
Major Intractable Java Bugs 
Timezones Issues
Recurring Events  
Notifications 
***Double-Bookings abounded 
No Doc for the existing Java Codebase 
** Bad Performance from JVM
    interactive-es 60 sec load time  
Inflexible 
No Client Based Solutions 
    So we took the Netscape approach, 
        and rewrote it?
Haha, might not want to phrase it that way - Netscape basically committed suicide this way.  Worked for skedge, though. 
--------------------
remaining issues after the rewrite were:
Cyclic Dependencies 
  GHC 
Scalable beyond 4 cores was not so good 
This was true at the time but not relevant since GHC 7.8.1. 
...
I'm not sure if these GC and stack trace things are relevant to folks from other platforms.
-----------------
Takeaway: 
...
Tens or maybe hundreds of thousands, now
Half the Client base has migrated 
All of the client base has migrated now
Testing ---> QuickCheck early on  
QA people find learning Haskell 
easier than other testing strategies
Well, there are easier QA tools.  But it was easier for them to write WebDriver tests in Haskell than in node.js, due to callback hell not existing in the Haskell version. 
SkedgeMe  
API into Sephora Website 
41-K.L.O.C. 
